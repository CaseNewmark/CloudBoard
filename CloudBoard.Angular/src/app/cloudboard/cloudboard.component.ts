...
export class CloudboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public isCloudBoardReady = false; // Add visibility control
  ...
  private loadCloudBoardById(cloudboardId: string): void {
    this.isLoading = true;
    this.isCloudBoardReady = false; // Hide content while loading
    ...
    this.cloudboardService.loadCloudBoardById(cloudboardId).subscribe({
      next: (cloudboard) => {
        ...
        setTimeout(() => {
          this.fCanvas()?.fitToScreen(undefined, false);
          this.isCloudBoardReady = true; // Show content after fit to screen
          this.changeDetectorRef.detectChanges();
        }, 500);
